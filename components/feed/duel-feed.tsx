...
import { FeaturedStrip, type CatalogMatch } from "./match-catalog";
...
export function DuelFeed({
  ...
  duels: Duel[];
  /** The full bettable catalogue — same shape "Jogos" uses — for this tab's
   *  own Destaques strip. What belongs there is "matches you can bet on,"
   *  not "matches someone already bet on": an admin-pinned match usually
   *  has no duel on it yet, so a strip built from `duels` alone would leave
   *  most manual pins never actually showing up here. */
  ...
  // Same reasoning as MatchCatalog's own ticking clock: a featured match's
  // tap behaviour (into bet creation vs. the "already started" explanation)
  // needs to flip the moment its kickoff passes, even if this tab has just
  // been sitting open.
  ...
  return (
    <div className="flex flex-col gap-3">
      {filter === "all" && <FeaturedStrip matches={matches} now={now} />}
